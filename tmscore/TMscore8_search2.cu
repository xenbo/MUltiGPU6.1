...
{
    volatile __shared__ short s1[150];
    volatile __shared__ short e1[150];
    volatile __shared__ short nj;
    int i, m;
    ...
    for(i=tid; i<nj; i=i+32)
    {
        float r1[l1][3],r2[l1][3];
        k=0;
        int kk;
		for(kk=s1[i]; kk<e1[i]; kk++)
        {
            r1[k][0]=xtm[kk][0];
            r1[k][1]=xtm[kk][1];
            r1[k][2]=xtm[kk][2];
